...
console.log(`Ваша оценка - 148-149 баллов 
Отзыв по пунктам ТЗ:
...
feedback: Бургер меню появляеться при уменьшении ширины страницы менее 500 пикселей, до этого присутствует фиксированное меню сверху страницы. 
Единственный нюанс, из-за фиксированного меню в шапке с position:fixed - якоря имеют сдвиг на размер этого самого меню, пока проблемку эту не решил, думаю цена этого косяка не более 1-2 баллов. 
Выполненные пункты:
...
feedback: Использованы теги: header, nav, footer, main, section, time + h1, h2, h3, h4
...
feedback: Доп инфа появляеться при наведении на картинку проекта
...
let timerID = setInterval(() => {
  if (!menuToggle.checked) clearTimeout(timerID);
  if (menuToggle.checked && body.clientWidth > 533) {
